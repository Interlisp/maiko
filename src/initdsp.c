...
init_display2(display_addr, display_max)
  INT   display_addr, display_max;
  {
	...
	exit( -1 );
      ...
      {
#ifdef KBINT
	/* int_io_open(LispWindowFd);  JDS 4/27/94 move to initkbd, to try preventing the move-mouse-never-get-kbd bug */
#endif
...
flush_display_buffer()
  {
   ...
   (currentdsp->bitblt_to_screen)(currentdsp,
				  DisplayRegion68k,
				  currentdsp->Vissible.x,
				  currentdsp->Vissible.y,
  ...
  int x ,y,w,h;
{
  ...
  longptr = (unsigned int *)
    (((UNSIGNED)((DLword*)DisplayRegion68k + (DLWORD_PERLINE * y))
			+ ((x +7)>>3)) & 0xfffffffc);
