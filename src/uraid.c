...
#ifdef DOS
char *URaid_summary1 = "<<Displaying the Stack>>\n\
c\t\t\tChecks all user stack contents\n\
...
char *URaid_summary2 = "<<Displaying memory contents>>\n\
a litatom\t\tDisplays the top-level value of the litatom\n\
...
char *URaid_summary3 = "<<Return or Exit>>\n\
e\t\t\tExit to DOS\n\
h\t\t\tDo a HARDRESET\n\
q\t\t\tReturns from URAID with NO change\n\
<<Misc>>\ns\t\t\tInvoke Shell\n\
...
device_after_raid()
  {
    ...
    init_ether();
#ifdef USE_DLPI
    if (ether_fd > 0) if (ioctl(ether_fd, I_SETSIG, S_INPUT) != 0)
      {
...
#ifdef XWINDOW
#ifdef I_SETSIG
    if (ioctl(ConnectionNumber(currentdsp->display_id), I_SETSIG, S_INPUT) < 0) perror("SETSIG on X fd failed");
#endif /* I_SETSIG */
#endif /* XWINDOW */
