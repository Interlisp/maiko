  ...
  for(j = 1; j <= Columns; j++)
	...
	while((!(frow[row_nr] && pcol[row_nr])) && row_nr <= Rows)
	  row_nr++; /* this sometimes sets row_nr to Rows + 1 and makes
        ...
        if(row_nr == Rows + 1)
          { printf("Inverting failed");  ERROR(ERR_NUM); }
	frow[row_nr] = FALSE;
		     ...
		     int    *sbas)
{
  ...
  if (st->saved)
    { if (st->saved == ST_SOLN) { st->saved = 0; return(OPTIMAL); } } 
  else if (SolveCount++ > 100) return(TIMEOUT);	/* Time out every 100 LP solves */
  else if ((KBDEventFlg>0) && *KEYBUFFERING68k == ATOM_T)
	return(TIMEOUT);	/* Time out on key/mouse clicks */
#ifdef DOS
  else if (currentmouse->Cursor.Moved) return(TIMEOUT);	/* Time out if mouse moves in DOS */
#endif /* DOS */
  ...
  else failure = OPTIMAL; /* Coming back thru after a timeout; we got OPTIMAL last time, so do it again. */
  ...
  if(failure == OPTIMAL)	/* there is a solution */
   ...
   else notint = st->notint; /* Coming back in, use old value. */
      ...
      if(notint)		/* there is at least one value not yet int */
	{
	  ...
	  memcpy(new_upbo,  upbo,  (Sum + 1) * sizeof(REAL));
          ...
          if(Floor_first)
            {
	      if (st->saved) new_bound = st->bound;
              else st->bound = new_bound = ceil(Solution[notint]) - 1;
              /* this bound might conflict */
	      if (st->saved >= ST_HI) resone = st->res1; /* We got the upper bound earlier; skip lower */
		  ...
		  Lp->eta_valid = FALSE;
		  st->saved = ST_LO;
	          resone = milpsolve(st, new_upbo, lowbo, new_basis, new_lower,
				     new_bas);
                  Lp->eta_valid = FALSE;
		  st->res1 = resone;
	      ...
	      else		/* bound feasible */
		{
		  ...
		  st->saved = ST_HI;
		  Lp->eta_valid = FALSE;
		  restwo = milpsolve(st, upbo, new_lowbo, new_basis, new_lower,
				     new_bas);
		  Lp->eta_valid = FALSE;
		  st->res2 = restwo;
		  ...
		  resone = MILP_FAIL;
	      ...
	      else		/* bound feasible */
	        {
	          new_lowbo[notint] = new_bound;
                  Lp->eta_valid = FALSE;
                  resone = milpsolve(st, upbo, new_lowbo, new_basis, new_lower,
				     new_bas);
                  Lp->eta_valid = FALSE;
	        }
              new_bound -= 1;
	      if(new_bound < lowbo[notint])
	      ...
	      else		/* bound feasible */
	        {
	          new_upbo[notint] = new_bound;
	          Lp->eta_valid = FALSE;
	          restwo = milpsolve(st, new_upbo, lowbo, new_basis, new_lower,
				     new_bas);
                  Lp->eta_valid = FALSE;
		}
	    }
