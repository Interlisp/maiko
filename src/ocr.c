...
LispPTR	*args;
{
	...
	switch (com) {
		...
		break;
	      ...
	      case DO_SCAN:
		{
			switch(ocr_scan((struct ocr_scan_para *)
					Addr68k_from_LADDR(args[1]))) {
			      case 1:
				val = ATOM_T;
				break;
		...
		if (OCR_sock < 0) {
			...
			OCR_iminfo_block
			  = (struct ocr_image_info *)Addr68k_from_LADDR(args[2]);
			
			switch (ocr_iminfo((struct ocr_up_para *)
					   Addr68k_from_LADDR(args[1]))) {
			      case 1:
				val = ATOM_T;
				break;
	      ...
	      case DO_IMG_UPLD:
		{
			...
			len = ocr_bulk_read(OCR_sock,
					    (char *)Addr68k_from_LADDR(bmp->bmbase),
					    10);
	      ...
	      case DO_CODE_CONV:
		{
			...
			ocr_code_conv((u_char *)Addr68k_from_LADDR(args[2]),
				      len);
			val = ATOM_T;
		}
		break;
		...
		break;
	      ...
	      default:
		error("ocr_comm: Invalid Comm byte");
	}
	return(val);
      INVAL:
	return(NIL);
...
struct ocr_scan_para	*sp;
{
	...
	if (write(OCR_sock, pkt, sizeof(pkt)) != sizeof(pkt)) {
		return 0;
	}
	if (write(OCR_sock, (char *)sp, sizeof(struct ocr_scan_para)) !=
...
struct ocr_up_para	*up;
{
	...
	if (write(OCR_sock, (char *)up, sizeof(struct ocr_up_para)) !=
...
ocr_img_upld_finish()
{
	...
	if (cnt == sizeof(pkt) && pkt[0] == 1) {
		S_TOUT_TIME(cnt = read(OCR_sock,
				       (char *)OCR_iminfo_block,
				       sizeof(struct ocr_image_info)), 5);
	...
	while (hd.len > 0) {
		if (buflen > 0) {
			...
			if (hd.len > 0) {
				...
				S_TOUT_TIME(cnt =
					   read(fd, &hd, sizeof(struct bd_header)), 1);
				continue;
			}
		} else {
...
static u_short	jis0[256] = 
{
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	...
	0x0078, 0x0079, 0x007A, 0x007B, 0x007C, 0x007D, 0x007E, 0x2223,
	...
	0x2223, 0x2123, 0x2156, 0x2157, 0x2122, 0x00B7, 0x2572, 0x2521,
	0x2523, 0x2525, 0x2527, 0x2529, 0x2563, 0x2565, 0x2567, 0x2544,
...
static u_short	jis1[256] = 
{
	...
	0x2223, 0x2121, 0x2122, 0x2123, 0x002C, 0x002E, 0x00B7, 0x003A,
	0x003B, 0x003F, 0x0021, 0x212B, 0x212C, 0x00C2, 0x00C1, 0x00C8,

	0x00C3, 0x2223, 0x00CC, 0x2133, 0x2134, 0x2135, 0x2136, 0x2137,
	0x2138, 0x2139, 0x213A, 0x213B, 0x213C, 0xEF24, 0x213E, 0x002F,

	0x005C, 0x007E, 0x003D, 0x007C, 0x2144, 0x003A, 0x00A9, 0x0027,
	0x00AA, 0x00BA, 0x0028, 0x0029, 0x214C, 0x214D, 0x005B, 0x005D,

	0x007B, 0x007D, 0xEF32, 0xEF33, 0x00AB, 0x00BB, 0x2156, 0x2157,
	0x2158, 0x2159, 0x215A, 0x215B, 0x002B, 0x002D, 0x00B1, 0x00B4,

	0x00B8, 0x003D, 0x2162, 0x003C, 0x003E, 0x2165, 0x2166, 0x2167,
	0x2168, 0x2169, 0x216A, 0x00B0, 0x216C, 0x216D, 0x216E, 0x00A5,

	0x00A4, 0x00A2, 0x00A3, 0x0025, 0x0023, 0x0026, 0x002A, 0x0040,
	0x00A7, 0x2179, 0x217A, 0x217B, 0x217C, 0x217D, 0x217E, 0x2223,
...
static u_short	jis2[256] = 
{
	...
	0x2223, 0x2221, 0x2222, 0x2223, 0x2224, 0x2225, 0x2226, 0x2227,
	0x2228, 0x2229, 0x00AE, 0x00AC, 0x00AD, 0x00AF, 0x222E, 0x2223,

	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0xEF4A, 0xEF4C, 0xEF59, 0xEF58, 0xEF5B, 0xEF5A,

	0xEF57, 0xEF56, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0xEFB6, 0xEFB7, 0xEF6A, 0xEF4F, 0xEF4E, 0xEFB5,

	0xEFB4, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0xEF6C, 0xEF70, 0x2223, 0xEFBA,

	0xEFB9, 0xEF72, 0x2223, 0xEF42, 0xEF43, 0x2223, 0x2223, 0xEF71,
	0xEF6F, 0xEF75, 0x226A, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0x2223, 0xF128, 0xEF41, 0x2223, 0x2223, 0x2223, 0xEF30,
	0xEF31, 0x00B0, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis3[256] = 
{
	...
	0x0030, 0x0031, 0x0032, 0x0033, 0x0034, 0x0035, 0x0036, 0x0037,
	0x0038, 0x0039, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0x0041, 0x0042, 0x0043, 0x0044, 0x0045, 0x0046, 0x0047,
	0x0048, 0x0049, 0x004A, 0x004B, 0x004C, 0x004D, 0x004E, 0x004F,

	0x0050, 0x0051, 0x0052, 0x0053, 0x0054, 0x0055, 0x0056, 0x0057,
	0x0058, 0x0059, 0x005A, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0x0061, 0x0062, 0x0063, 0x0064, 0x0065, 0x0066, 0x0067,
	0x0068, 0x0069, 0x006A, 0x006B, 0x006C, 0x006D, 0x006E, 0x006F,

	0x0070, 0x0071, 0x0072, 0x0073, 0x0074, 0x0075, 0x0076, 0x0077,
	0x0078, 0x0079, 0x007A, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis6[256] = 
{
	...
	0x2223, 0x2641, 0x2642, 0x2644, 0x2645, 0x2646, 0x2649, 0x264A,
	0x264B, 0x264C, 0x264D, 0x264E, 0x264F, 0x2650, 0x2651, 0x2652,

	0x2653, 0x2655, 0x2656, 0x2658, 0x2659, 0x265A, 0x265B, 0x265C,
	0x265D, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0x2661, 0x2662, 0x2664, 0x2665, 0x2666, 0x2669, 0x266A,
	0x266B, 0x266C, 0x266D, 0x266E, 0x266F, 0x2670, 0x2671, 0x2672,

	0x2673, 0x2675, 0x2676, 0x2678, 0x2679, 0x267A, 0x267B, 0x267C,
	0x267D, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis84[256] = 
{
	...
	0x2223, 0x7521, 0x7522, 0x7523, 0x7524, 0x7525, 0x7526, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis8[256] = 
{
	...
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0xEF69, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0xF128, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,

	0x2223, 0x2223, 0x2223, 0x2223, 0x7421, 0x7421, 0xEFFC, 0xEFFC,
	0x2220, 0x2220, 0xEFA8, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis9[256] = 
{
	...
	0x2223, 0x2223, 0x2223, 0x2223, 0xEFD1, 0xEFD2, 0xEFD3, 0xEFD4,
	0xEFD5, 0xEFD6, 0xEFD7, 0xEFD8, 0xEFD9, 0xEFDA, 0x2223, 0x2223,

	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0xEFD1, 0xEFD2, 0xEFD3, 0xEFD4, 0xEFD5, 0xEFD6, 0xEFD7,

	0xEFD8, 0xEFD9, 0xEFDA, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0xEFC2,

	0xEFC3, 0xEFC4, 0x2223, 0xEFC6, 0xEFC7, 0xEFC8, 0xEFC9, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis10[256] = 
{
	...
	0x2223, 0x2223, 0x2223, 0x2223, 0x7409, 0x7421, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
...
static u_short	jis14[256] = 
{
	...
	0x2223, 0x2122, 0x2123, 0x2124, 0x2125, 0x2223, 0x2223, 0x213C,
	0xEF24, 0x213E, 0x213B, 0x2223, 0x2223, 0x2144, 0x2145, 0x00A9,

	0x0027, 0x00AA, 0x00BA, 0x0028, 0x0029, 0x214C, 0x214D, 0x005B,
	0x005D, 0x007B, 0x007D, 0xEF32, 0xEF33, 0x00AB, 0x00BB, 0x2156,

	0x2157, 0x2223, 0x2223, 0x215A, 0x215B, 0x2223, 0x2223, 0x2223,
	0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223, 0x2223,
	...
	while (len > 0) {
		if (*ptr == 0xFF) { /* ASCII & Hankaku-kana */
			*(u_short *)ptr = jis0[*(ptr + 1)];
			...
			switch (*ptr) {
			      case 0X21: /* 1 Ku */
				*(u_short *)ptr = jis1[*(ptr + 1)];
				break;
			      case 0X22: /* 2 Ku */
				*(u_short *)ptr = jis2[*(ptr + 1)];
				break;
			      case 0X23: /* 3 Ku */
				*(u_short *)ptr = jis3[*(ptr + 1)];
				break;
			      case 0X26: /* 6 Ku */
				*(u_short *)ptr = jis6[*(ptr + 1)];
				break;
			      case 0X28: /* 8 Ku */
				*(u_short *)ptr = jis8[*(ptr + 1)];
				break;
			      case 0X29: /* 9 Ku */
				*(u_short *)ptr = jis9[*(ptr + 1)];
				break;
			      case 0X2A: /* 10 Ku */
				*(u_short *)ptr = jis10[*(ptr + 1)];
				break;
			      case 0X2E: /* 14 Ku */
				*(u_short *)ptr = jis14[*(ptr + 1)];
				break;
			      case 0X74: /* 84 Ku */
				*(u_short *)ptr = jis84[*(ptr + 1)];
				break;
			      default:
				break;
			}
