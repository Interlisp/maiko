...
unsigned long set_DAC_color(args)
    LispPTR args[];
{
  union REGS inregs, outregs;
     ...
     DspInterface dsp;
{
  ...
  case 64:
    dsp->DisplaySegMagnitude = 0x10;
    break;
  case 128:
    dsp->DisplaySegMagnitude = 0x11;
    break;
  case 256:
    dsp->DisplaySegMagnitude = 0x12;
    break;
  }
...
unsigned long tmpbbt(dsp, buffer, left, top, swidth, height)
     DspInterface dsp;
     char *buffer;
     ...
     int width_hint, height_hint, depth_hint;
{
  ...
  if ( dsp->graphicsmode == 0 ) {
	...
	else VESA_errorexit("Displaymode not supported", -1);
	break;
      case 8:
	...
	break;
      default:
	VESA_errorexit("Requested display depth not supported", -1);
      }
    }
