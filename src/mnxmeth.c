...
intersectregions(reg1, reg2, result)
     MRegion *reg1, *reg2, *result;
{
  result->x = max(reg1->x, reg2->x);
  result->y = max(reg1->y, reg2->y);
  result->width = result->x - min(reg1->x + reg1->width,
				  reg2->x + reg2->width);
  result->height = result->y - min(reg1->y + reg1->height,
				   reg2->y + reg2->height);
}
...
/**************************************************************/
int getwhiteborder( medleywindow )
     LispPTR medleywindow;
{
  int border;
...
/* Takes a medley window as its argument.                     */
/**************************************************************/
int getblackborder( medleywindow )
     LispPTR medleywindow;
{
  int border;
...
/* and X.                                                     */
/**************************************************************/
void setlineattributes( display, dspif, wif, dd, mwidth, mdash )
  Display *display;
  DspInterface dspif;
  WindowInterface wif;
  DISPLAYDATA *dd;
...
settileorigin(medleywin, x, y)
  LispPTR medleywin, x, y;
{
  WindowInterface wif;
  Display *display;
...
/**************************************************************/
/**************************************************************/
calculateshape( wif, x, y, width, height, topwidoffset, tophgtoffset)
  WindowInterface wif;
  ...
  int xoff, yoff;
{
  ...
  wif->topregion.y = (HeightOfScreen( wif->screen )
			- wif->windowreg.y
			- wif->windowreg.height); 
  ...
  sprintf(wif->gstring, "%dx%d+%d+%d",
	  wif->topregion.width,
	  wif->topregion.height,
	  wif->topregion.x,
	  wif->topregion.y);
}
     ...
     LispPTR medleywin;
{
  ...
  XtVaSetValues(wif->formwidget,
		XtNclippX, dd->ddclippingleft,
		XtNclippY, dd->ddclippingbottom,
		XtNclippWidth, dd->ddclippingright - dd->ddclippingleft,
		XtNclippHeight, dd->ddclippingtop - dd->ddclippingbottom,
		NULL);
  if(XtIsRealized(wif->topwidget)){
    XtConfigureWidget(wif->topwidget,
		      wif->topregion.x,
		      wif->topregion.y,
		      wif->topregion.width,
		      wif->topregion.height, 0);

    XtConfigureWidget(wif->framewidget,
		      wif->outerregion.x,
		      wif->outerregion.y,
		      wif->outerregion.width,
		      wif->outerregion.height, 0);

    XtConfigureWidget(wif->windowwidget,
		      wif->innerregion.x,
		      wif->innerregion.y,
		      wif->innerregion.width,
		      wif->innerregion.height,
		      wif->whiteborder);
  }
}
...
int translate_x(wif, xval)
  int xval;
  WindowInterface wif;
  {
    return (xval + (wif->xoffset - wif->windowreg.x
							- (wif->blackborder2>>1) - wif->whiteborder ));
  }



int translate_y(wif, yval)
  int yval;
  WindowInterface wif;
  {
    return (yval + wif->yoffset 
			  -  (wif->windowreg.y + (wif->blackborder2>>1) + wif->whiteborder));
  }
...
/**************************************************************/
InitScratchDepth(image, MBM, scdepth)
     XImage *image; 
     BITMAP *MBM;
     int scdepth;
{ /* Set up the scratch image for this operation */
  ...
  if((MBM->bmbitperpixel & 0xFFFF) == 1)
  ...
  else/* if(image->format != ZPixmap) */
    {
      ...
      _XInitImageFuncPtrs(image);
      image->bytes_per_line =
	((((image->width * image->depth)+(BITSPER_DLWORD-1))
	  / BITSPER_DLWORD)
	 * (BITSPER_DLWORD/8));
    }
}
...
MakeScratchImageFromBM(dspif, bitmap)
     ...
     BITMAP *bitmap;
{
  InitScratchDepth(&dspif->image, bitmap,
		   DefaultDepthOfScreen(dspif->xscreen));
...
MakeScratchImageFromInt(dspif, Texture)
     DspInterface dspif;
     int Texture;
{
     ...
     LispPTR medleywin;
{
  ...
  bgpixmap = XCreatePixmapFromBitmapData
    (display,
     ...
     dspif->image.width,
     dspif->image.height,
     WhitePixelOfScreen(wif->screen),
     BlackPixelOfScreen(wif->screen), 
...
MBMToDrawable(MBM, dspif, wif, srcx, srcy, dstx, dsty, width, height, operation)
     BITMAP *MBM;
     WindowInterface wif;
     DspInterface dspif;     
     int srcx, srcy, dstx, dsty, width, height, operation;
{
  ...
  if((MBM->bmbitperpixel == 1) ||
     (MBM->bmbitperpixel == DefaultDepthOfScreen(wif->screen)))
    {
      InitScratchDepth(&dspif->image, MBM, DefaultDepthOfScreen(wif->screen));
      dspif->image.data = (char *)Cptr((LispPTR)MBM->bmbase);
      switch(operation)
		{
		  case REPLACE: if (window) XPutImage(dspif->handle, window, wif->ReplaceGC,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
		                XPutImage(dspif->handle, wif->backing, dspif->PixRGC,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
						break;

		  case INVERT:  if (window) XPutImage(dspif->handle, 
								  window, wif->InvertGC,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
		                XPutImage(dspif->handle, wif->backing, dspif->PixIGC,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
						break;

		  case PAINT:   if (window) XPutImage(dspif->handle, window, wif->PaintGC1,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
                        if (window) XPutImage(dspif->handle, window, wif->PaintGC2,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
		                XPutImage(dspif->handle, wif->backing, dspif->PixPGC,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
						break;

		  case ERASE:   if (window) XPutImage(dspif->handle, window, wif->EraseGC1,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
                        if (window) XPutImage(dspif->handle, window, wif->EraseGC2,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
		                XPutImage(dspif->handle, wif->backing, dspif->PixEGC,
		                          &dspif->image, srcx, srcy, dstx, dsty,
								  width, height);
						break;
        }
    ...
    dspif->image.data = (char *)malloc(dspif->image.bytes_per_line
				       * dspif->image.width
				       * dspif->image.depth);
	...
	{
	  XPutImage(dspif->handle, window, wif->InvertGC,
		    &dspif->image, srcx,srcy,dstx,dsty,width,height);
	}
      else if( operation == ERASE_atom )
	{
	  XPutImage(dspif->handle, window, wif->EraseGC1,
		    &dspif->image, srcx,srcy,dstx,dsty,width,height);
	  XPutImage(dspif->handle, window, wif->EraseGC2,
		    &dspif->image, srcx,srcy,dstx,dsty,width,height);
	}
      else if( operation == PAINT_atom )
	{
	  XPutImage(dspif->handle, window, wif->PaintGC1,
		    &dspif->image, srcx,srcy,dstx,dsty,width,height);
	  XPutImage(dspif->handle, window, wif->PaintGC2,
		    &dspif->image, srcx,srcy,dstx,dsty,width,height);
	}
      else if( operation == REPLACE_atom )
	{
	  XPutImage(dspif->handle, window, wif->ReplaceGC,
		    &dspif->image, srcx,srcy,dstx,dsty,width,height);
	}
...
/************************************************************************/
DrawableToMBM(MBM, dspif, wif, xdrawable, xscreen, srcx, srcy, dstx, dsty, width, height, op)
     BITMAP *MBM;
	 WindowInterface wif;
     Screen *xscreen;
     Drawable xdrawable;
     ...
     int srcx, srcy, dstx, dsty, width, height, op;
{
  ...
  if(MBM->bmbitperpixel == DefaultDepthOfScreen(xscreen))
    ...
    dspif->tmpimage.data = (char *)malloc(dspif->tmpimage.bytes_per_line
					  * dspif->tmpimage.width
...
MNXBBTToXWindow( args )
     LispArgs args; /* args[0] = LispPTR to MedleyWindow */
			...
			/* args[9] = operation */
{
  ...
  MBMToDrawable(bitmap, dspif, wif,
		LispIntToCInt(args[2]), /* src x */
		srcy,
		translate_x(wif, LispIntToCInt(args[5])), /* dst x */
...
MNXBBTFromXWindow( args )
     LispArgs args; /* args[0] = LispPTR to src WINDOW */
		    /* args[1] = scr_x */
		    /* args[2] = scr_y */
     ...
     /* The handling of the OPERATION argument is done in lisp */
{
  ...
  DrawableToMBM(bitmap, dspif, wif, wif->backing, wif->screen, srcx, srcy, dstx, dsty, width, height, args[8]&0xF);
     ...
     /* Things to remember: This function does not handle integer-textures */
{
  ...
  switch (args[8] & 0xF)
	{
	  case INVERT:   XCopyArea( display, swin, dwin, srcwif->InvertGC,
                                srcx, srcy, width, height, dstx, dsty);
                     XCopyArea(display, srcwif->backing, dstwif->backing,
                               dspif->PixIGC, srcx, srcy, width, height,
                               dstx, dsty);
                     break;

	  case ERASE:    XCopyArea(display, swin, dwin, srcwif->EraseGC1,
                               srcx, srcy, width, height, dstx, dsty);
                     XCopyArea(display, swin, dwin, srcwif->EraseGC2,
                               srcx, srcy, width, height, dstx, dsty);
	                 XCopyArea(display, srcwif->backing, dstwif->backing,
							   dspif->PixEGC, srcx, srcy, width, height,
							   dstx, dsty);
                     break;

	  case PAINT:    XCopyArea(display, swin, dwin, srcwif->PaintGC1,
                               srcx, srcy, width, height, dstx, dsty);
                     XCopyArea(display, swin, dwin, srcwif->PaintGC2,
                               srcx, srcy, width, height, dstx, dsty);
                     XCopyArea(display, srcwif->backing, dstwif->backing,
                               dspif->PixPGC, srcx, srcy, width, height,
							   dstx, dsty);
                     break;

	  case REPLACE:  XCopyArea(display, swin, dwin, srcwif->ReplaceGC,
                               srcx, srcy, width, height, dstx, dsty);
                     XCopyArea(display, srcwif->backing, dstwif->backing,
                               dspif->PixRGC, srcx, srcy, width, height,
                               dstx, dsty);
                     break;
    }
     ...
     LispArgs args;
{
  ...
  DrawableToMBM(bitmap, dspif, wif, wif->backing, wif->screen, 0, 0,
		/* wif->innerregion.x, wif->innerregion.y, */ 0, 0,
		wif->innerregion.width, wif->innerregion.height, REPLACE);
  XtUnrealizeWidget(wif->topwidget);

  XtRemoveEventHandler(wif->framewidget,
		    ButtonPressMask | ButtonReleaseMask,
		    False, HandleButton, wif);
  XtRemoveEventHandler(wif->framewidget,
		    PointerMotionMask,
		    False, HandleMotion, wif);
  XtRemoveEventHandler(wif->framewidget,
		    KeyPressMask | KeyReleaseMask,
		    False, HandleKey, wif);
  XtRemoveEventHandler(wif->framewidget,
		    EnterWindowMask | LeaveWindowMask,
		    False, HandleCrossing, wif);
  XtRemoveEventHandler(wif->framewidget,
		    FocusChangeMask,
		    False, HandleFocus, wif);
  XtRemoveEventHandler(wif->framewidget,
		    ExposureMask,
		    False, HandleTitle, wif);
  XtRemoveEventHandler(wif->windowwidget,
		    ExposureMask,
		    False, HandleExpose, wif);
  XtRemoveEventHandler(wif->topwidget,
		    StructureNotifyMask,
		    False, HandleStructure, wif);
     ...
     WindowInterface chain, wif;
{
  ...
  if(chain == (WindowInterface)NULL)
    return(NULL);
  else if(chain == wif) return(chain->next);
  else {
     ...
     WindowInterface wif;
{
  ...
  for(prev = dspif->CreatedWifs, curr = prev->next;
      ((curr != (WindowInterface)NULL) && (curr != wif));
...
MNXdestroyDisplay( args )
     LispArgs args; /* args[0] = LispPTR to MedleyScreen */
{
  DspInterface dspif;
  WindowInterface i;
                    ...
                    /* args[4] = height */
{
  ...
  if (wif->open)
	{
      ...
      XtVaSetValues(wif->formwidget, XtNextentX, 0, XtNextentY, 0,
                    XtNextentWidth, args[3] & 0xFFFF,
				    XtNextentHeight, args[4] & 0xFFFF,
				    NULL);
      XtMoveWidget(wif->topwidget, args[1] & 0xFFFF,
                   HeightOfScreen(wif->screen) -
						  (args[2] & 0xFFFF) - wif->topregion.height);
    }
  else
	{
	  wif->shape_pend = 1;
  ...
  wif->backing = XCreatePixmap(dsp, wif->dspif->root,
									args[3] & 0xFFFF, args[4] & 0xFFFF, 1);
		    ...
		    /* args[5] = Iconwindow height */
{
  ...
  DrawableToMBM(bitmap, dspif, wif, wif->backing, XtScreen(wif->topwidget), 0, 0, 0, 0,
		wif->innerregion.width, wif->innerregion.height, REPLACE);
  XIconifyWindow( XtDisplay(wif->topwidget)
		 , XtWindow(wif->topwidget)
     ...
     LispArgs args; /* args[0] = LispPTR to MedleyWindow */
{
  ...
  showtitle(args[0]);
  bitmap = (BITMAP *)Cptr(((MedleyWindow)Cptr(args[0]))->SAVE);
  MBMToDrawable(bitmap, dspif, wif, 0, 0, 0, 0,
		    ...
		    /* args[4] = height */
{
  ...
  /********************************/
  wif->topwidget = XtVaAppCreateShell
    (tmpstring, tmpstring,
     applicationShellWidgetClass, display,
     XtNgeometry, wif->gstring,
     XtNargc, save_argc,
     XtNargv, save_argv,
     XtNborderWidth, 0,
     NULL);

  wif->formwidget = XtCreateManagedWidget
    ("scrollBox", scrollBoxWidgetClass, wif->topwidget,
     NULL, 0);

  wif->framewidget = XtVaCreateManagedWidget
    ("frame", boxWidgetClass, wif->formwidget,
     XtNheight, wif->outerregion.height,
     XtNwidth, wif->outerregion.width,
     XtNborderWidth, blackborder,
     XtNbackground, BlackPixelOfScreen( wif->screen ),
     XtNvSpace, 0,
  ...
  wif->windowwidget = XtVaCreateManagedWidget
    ("window", boxWidgetClass, wif->framewidget,
     XtNheight, wif->innerregion.height,
     XtNwidth, wif->innerregion.width,
     XtNx, wif->innerregion.x,
     XtNy, wif->innerregion.y,
     XtNborderColor, WhitePixelOfScreen(wif->screen),
     XtNborderWidth, whiteborder,
     XtNvSpace, 0,
     XtNdefaultDistance, 0,
     XtNhorizDistance, 0,
     XtNvertDistance, 0,
  ...
  wif->ReplaceGC = XCreateGC( display, dspif->root
  ...
  gcv.background = 0;
  wif->InvertGC = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = dspif->black & (~ dspif->white);
  gcv.background = 0;
  wif->PaintGC1 = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.background = 0;
  wif->PaintGC2 = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = dspif->white & (~dspif->black);
  gcv.background = 0;
  wif->EraseGC1 = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = dspif->black & (~dspif->white);
  gcv.background = 0;
  wif->EraseGC2 = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = dspif->black & (~dspif->white);
  gcv.background = 0;
  wif->gc = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  wif->backing = XCreatePixmap(display, dspif->root,
                               args[3] & 0xFFFF, args[4] & 0xFFFF, 1);
  bitmap = (BITMAP *)Cptr(((MedleyWindow)Cptr(args[0]))->SAVE);
     ...
     LispPTR win;
{
  ...
  if((bitmap->bmbitperpixel == 1) ||
     (bitmap->bmbitperpixel == DefaultDepthOfScreen(wif->screen)))
    for (i = 0; i < stringlen; i++) {
      width = GETWORD((DLword*)Cptr(dd->ddwidthscache + tmpstring[i]));
      XPutImage(dspif->handle,
		wif->blackframe,
		dspif->TitleGC,
		&dspif->image,
     ...
     LispArgs args;
{
  ...
  XtAddEventHandler(wif->framewidget,
		    ButtonPressMask | ButtonReleaseMask,
		    False, HandleButton, wif);
  XtAddEventHandler(wif->framewidget,
		    PointerMotionMask,
  ...
  XtAddEventHandler(wif->framewidget,
		    ExposureMask,
		    False, HandleTitle, wif);
  XtAddEventHandler(wif->windowwidget,
		    ExposureMask,
		    False, HandleExpose, wif);
  XtAddEventHandler(wif->topwidget,
		    StructureNotifyMask,
		    False, HandleStructure, wif);
  ...
  if (wif->shape_pend)		/* We reshaped while closed; do it for real here */
	{
      refreshwindow( args[0] );
      XtVaSetValues(wif->formwidget, XtNextentX, 0, XtNextentY, 0,
                    XtNextentWidth, wif->windowreg.width,
				    XtNextentHeight, wif->windowreg.height,
				    NULL);
  ...
  if (!wif->backing)
	{
	  wif->backing = XCreatePixmap(display, window, wif->outerregion.width,
                               wif->outerregion.height, 1);
    }
...
MNXSetOffsets(args)
  LispArgs args;   /* args[0] = window XOFFSET/YOFFSET changed in */
				   /* args[1] = new XOFFSET */
				   /* args[2] = new YOFFSET */
  {
    Display *display;
		    ...
		    /* args[1] = Smallp for function. */
{
  ...
  if (dd->ddoperation == REPLACE_atom) wif->op = 0;
  else if (dd->ddoperation == PAINT_atom) wif->op = 1;
  else if (dd->ddoperation == INVERT_atom) wif->op = 2;
  else if (dd->ddoperation == ERASE_atom) wif->op = 3;
                    ...
                    /* args[1] = LispPTR to FIXP or BITMAPP */
{
  ...
  if(dspif->image.depth == 1) {
    wif->fgpixmap = XCreatePixmapFromBitmapData(dspif->handle,
						XtWindow(wif->windowwidget),
						dspif->image.data,
						dspif->image.width,
						dspif->image.height,
						0, 
...
MNXBitBltBW(  )
{
  error("Call to MNXBitBltBW. This function is not in use anymore.");
}
		    ...
		    /* args[7] = background, fixp or bitmap */
{
  ...
  dsty = wif->innerregion.height 
		  - translate_y(wif, LispIntToCInt(args[3])) - regheight;
...
bltshade_internal(display, op, texture, dstx, dsty, width, regheight, wif, dspif)
  Display *display;
  LispPTR texture;
  ...
  DspInterface dspif;
{
  ...
  if(TYPE_SMALLP == GetTypeNumber(texture))
	...
	{			/* Medley sez' color the foreground */
  ...
  if(dspif->image.depth == 1)
    {
#endif /* NEVER */
      bgpixmap = XCreatePixmapFromBitmapData(dspif->handle,
					     XtWindow(wif->windowwidget),
					     dspif->image.data,
					     dspif->image.width,
					     dspif->image.height,
  ...
  switch(op)
    ...
    case 0:
      ...
      XFillRectangle(display, wif->backing, dspif->PixRGC,
    ...
    case 2:
      ...
      XFillRectangle(display, wif->backing, dspif->PixIGC,
                     dstx, dsty, width, regheight);
      break;
    case 3:
      ...
      XFillRectangle(display, wif->backing, dspif->PixEGC,
                     dstx, dsty, width, regheight);
      break;
    }
...
/*   XSetTSOrigin(display, wif->gc, dstx, dsty);
  ...
  XCopyPlane(display, wif->backing, XtWindow(wif->windowwidget), wif->ReplaceGC,
					  dstx, dsty, width, regheight, dstx, dsty, 1);

  XFreePixmap(display, bgpixmap);
  switch(op)			/* undo the changing of foreground color above */
    {
    case 0:	
    case 1: XSetFillStyle(display, dspif->PixRGC, FillSolid);
			break;
    case 2: XSetForeground(display, wif->gc, dspif->black);
			XSetFillStyle(display, dspif->PixIGC, FillSolid);
			break;
    case 3: XSetForeground(display, wif->gc, dspif->black);
...
XClearToEOL ( args )
     LispArgs args; /* args[0] = MedleyWindow */
{
  Display *display;
  Window window;
  WindowInterface wif;
  ...
  if( dd->ddoperation == INVERT_atom )
    {
      ...
      XPutImage( display, wif->backing, dspif->PixIGC, &dspif->image, sx, sy, cx, cy, w, bitmap->bmheight );
    }
  else if( dd->ddoperation == ERASE_atom )
    {
      ...
      XPutImage( display, wif->backing, dspif->PixEGC, &dspif->image, sx, sy, cx, cy, w, bitmap->bmheight );
    }
  else if( dd->ddoperation == PAINT_atom )
    {
      ...
      XPutImage( display, wif->backing, dspif->PixPGC, &dspif->image, sx, sy, cx, cy, w, bitmap->bmheight );
    }
  else if( dd->ddoperation == REPLACE_atom )
    {
      XPutImage( display, window, wif->ReplaceGC, &dspif->image, sx, sy, cx, cy, w, bitmap->bmheight );
      XPutImage( display, wif->backing, dspif->PixRGC, &dspif->image, sx, sy, cx, cy, w, bitmap->bmheight );
    }
...
/************************************************************/
MNXgetwindowprop( args )
     LispArgs args; /* args[0] = LispPTR to MedleyWindow */
{
}

MNXputwindowprop( args )
     LispArgs args; /* args[0] = LispPTR to MedleyWindow */
                    ...
                    /* args[n] = args for the method. */
{
  ...
  case MNWTitle:
    ...
    break;
  case MNWScrollFn:
    XtVaSetValues(WIfFromMw(args[0])->formwidget,
                  XtNbarOnOff, (args[1] != NIL), 
				  XtNmedleyWindow, wif,
				  XtNhScroll, &SignalHScroll,
				  XtNhJmpScroll, &SignalHJmpScroll,
				  XtNvScroll, &SignalVScroll,
				  XtNvJmpScroll, &SignalVJmpScroll,
				  NULL);
  ...
  case MNWScrollExtent:  
    XtVaSetValues(WIfFromMw(args[0])->formwidget,
		  XtNextentX, LispIntToCInt( args[1] ),
		  XtNextentY, LispIntToCInt( args[2] ),
		  XtNextentWidth, LispIntToCInt( args[3] ),
		  XtNextentHeight, LispIntToCInt( args[4] ),
		  NULL);
    break;
...
MNXGrabPointer(args)
  ...
  if (args[1])
	{
	  ...
	  bits = XCreatePixmapFromBitmapData(display, dspif->root, srcbits,
										 16, 16, 1, 0, 1);
  ...
  res = XGrabPointer(display, promptw, False, PointerMotionMask|ButtonPressMask|ButtonReleaseMask,
							  GrabModeAsync, GrabModeAsync,
							  None, grab_cursor, CurrentTime);


  if (grab_cursor != None) { XFreePixmap(display, bits); }
  if (res == GrabSuccess) return (ATOM_T);
  else return(NIL);
...
MNXDrawBox(args)
  LispArgs args;	/* args[0] = Medley SCREEN to draw box on */
					/* args[1] = Left of box */
...
MNXMovePointer( args )
     LispArgs args; /* args[0] = LispPTR to SCREEN */
  		    ...
  		    /* args[1] = NIL for GC off, non-NIL for GC on */
{
  ...
  if(dspif->gcindicator)
	...
	XFillRectangle(XtDisplay(dspif->gcindicator), XtWindow(dspif->gcindicator),
		       DefaultGCOfScreen(XtScreen(dspif->gcindicator)), 0, 0,
		       8,
		       8);
}
...
MNXMakePromptWindow( args )
     LispArgs args; /* args[0] = Lisp window, the prompt window */
{
  Widget form, menu;
  DspInterface dspif;
  WindowInterface wif;
  ...
  dspif->legatewidget = XtVaAppCreateShell
    ("Medley Status", "Medley Status",
     applicationShellWidgetClass, XDisplayFromMw(args[0]),
     XtNargc, save_argc,
     XtNargv, save_argv,
     XtNborderWidth, 0,
     NULL);
  ...
  dspif->gcindicator = XtVaCreateManagedWidget
    ("gcindicator", boxWidgetClass, form,
     XtNwidth, 8,
     XtNheight, 8,
     NULL);
  ...
  XtAddEventHandler(menu,
		    ButtonPressMask | ButtonReleaseMask,
		    False,
		    HandleBackgroundButton,
		    dspif->screen);
  XtAddEventHandler(dspif->legatewidget,
		    PointerMotionMask,
		    False, HandleMotion, 0);
  XtAddEventHandler(dspif->legatewidget,
		    EnterWindowMask | LeaveWindowMask,
		    False, HandleBackgroundCrossing, dspif);
  XtAddEventHandler(dspif->legatewidget,
		    KeyPressMask | KeyReleaseMask,
		    False, HandleKey, dspif);
...
MNXSetCursor(args)
   LispArgs args;   /* args[0] = Medley screen */
					...
					/* args[3] = hot-spot y */
  {
	...
	bits = XCreatePixmapFromBitmapData(display, dspif->root, srcbits,
									   16, 16, 1, 0, 1);
	new_cursor = XCreatePixmapCursor(display, bits, bits, &dspif->black,
									  &dspif->white, args[2]&0xFFFF, args[3]&0xFFFF);
...
/************************************************************/
InitDsp( args )
     LispArgs args; /* arg[0] = LispPTR to MedleyScreen */
{
  DspInterface dspif;
  MedleyScreen SCREEN;
  ...
  if(LispStringP (SCREEN->NATIVE_INFO))
    {
      ...
      dspif->handle = XtOpenDisplay( dspif->xtcontext,
				    tmpstring,
				    NULL,
				    "window",
				    NULL,
  ...
  else dspif->handle = XtOpenDisplay( dspif->xtcontext,
				     NULL,
				     NULL,
				     "window",
				     NULL,
  ...
  dspif->TitleGC = XCreateGC( display, dspif->root
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = 1;
  gcv.background = 0;
  dspif->PixRGC = XCreateGC( display, tpx
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = 1;
  gcv.background = 0;
  dspif->PixPGC = XCreateGC( display, tpx
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = 1;
  gcv.background = 0;
  dspif->PixIGC = XCreateGC( display, tpx
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  gcv.foreground = 1;
  gcv.background = 0;
  dspif->PixEGC = XCreateGC( display, tpx
			     , GCForeground | GCBackground | GCFunction
			     , &gcv );
  ...
  SCREEN->SCWIDTH =
    CIntToLispInt(WidthOfScreen(screen));
  SCREEN->SCHEIGHT =
    CIntToLispInt(HeightOfScreen(screen));
#ifndef NEVER
  SCREEN->SCDEPTH =
    CIntToLispInt(PlanesOfScreen(screen));
#endif
  ...
  XSynchronize(display, True);
	...
	if (diff)
	  for (mask = 1; mask; mask<<= 1) { if (mask & diff) break;}
    else
	  {
		mask = 1; inverted = 1;
    ...
    dspif->GetRGC = XCreateGC( display, dspif->root
			       , GCForeground | GCBackground | GCFunction
			       , &gcv );

    gcv.function   = GXxor;
    dspif->GetIGC = XCreateGC( display, dspif->root
			       , GCForeground | GCBackground | GCFunction
			       , &gcv );

    gcv.function   = GXor;
    dspif->GetPGC = XCreateGC( display, dspif->root
			       , GCForeground | GCBackground | GCFunction
			       , &gcv );

    gcv.function   = GXandInverted;
    dspif->GetEGC = XCreateGC( display, dspif->root
			       , GCForeground | GCBackground | GCFunction
			       , &gcv );
    dspif->bw_plane = mask;
	...
	gcv.foreground = 255;
	gcv.background = 0;
    dspif->BoxingGC = XCreateGC( display, dspif->root
			       , GCForeground | GCBackground | GCFunction |
     ...
     LispPTR *args;
{
  ...
  if(type == MedleyScreenType) { return(InitDsp(args)); }
  else error("Bogus type\n");
  return(ATOM_T);
}
