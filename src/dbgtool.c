...
#define BT_morep if((BT_temp != '!') && (++BT_lines>BTMAXLINE)){\
	...
	while(nfvar && nfvar-- >= max_npvar)
	  {
		...
		print_atomname(get_fvar_name(fx_addr68k , i));
		printf("  top value ]   ");
		print(*(LispPTR *)(Addr68k_from_LADDR(((int)(0xFFF & GETWORD(ptrlo)) << 16) | GETWORD(ptrhi))));
		putchar('\n');BT_morep;
	      }
	    ptr += 2 ;
	    i++;
