...
typedef struct first_array {
		...
		unsigned short offset ; } FirstArray;
	...
	int             args[];
{
	...
	case WNN_CONV:
		...
		for (i = length; i < (CHAR_MAXLEN+1)*2; i++)
			c_char1[i] = NULL;
	...
	case WNN_GET_KOUHO:
		...
		base1 = ((unsigned int *) (Addr68k_from_LADDR((unsigned int) a_ptr->base))) + (int)(a_ptr->offset);
	...
	case WNN_GET_USERDIC:
		...
		a_ptr = (FirstArray *) (Addr68k_from_LADDR((unsigned int) args[2]));
		base2 = ((unsigned int *) (Addr68k_from_LADDR((unsigned int) a_ptr->base))) + (int)(a_ptr->offset);

		a_ptr = (FirstArray *) (Addr68k_from_LADDR((unsigned int) args[3]));
		base3 = ((unsigned int *) (Addr68k_from_LADDR((unsigned int) a_ptr->base))) + (int)(a_ptr->offset);
...
ejlisp_add_userdic (dic_no,kanji,yomi,hinshi)
int	dic_no;
...
int	hinshi;
{
	...
	if (result == 0) {
		TIMEOUT(jl_dic_save (ejlisp_buf,dic_no));
	}
...
ejlisp_bunsetsu_kanji(bunsetsu_no,bunsetsu_no2,kanji_len)
int	bunsetsu_no,bunsetsu_no2;
int	*kanji_len;
{
...
ejlisp_bunsetsu_yomi(bunsetsu_no,bunsetsu_no2,yomi_len)
int	bunsetsu_no,bunsetsu_no2;
int	*yomi_len;
{
...
ejlisp_conv(yomi,kanji,kanji_len,bunsetsu_suu)
unsigned short	*yomi;
unsigned short	*kanji;
int	*kanji_len,*bunsetsu_suu;
{
	...
	if (*bunsetsu_suu != -1) {
		TIMEOUT(*kanji_len = jl_get_kanji (ejlisp_buf,0,-1,kanji)) ;
	}
...
ejlisp_conv_end ()
{
	...
	if (wnn_errorno == 0) {
		TIMEOUT(jl_dic_save_all (ejlisp_buf));
	}
...
ejlisp_get_kouho(pos,kouho_arrey,kouho_num)
unsigned short	kouho_arrey[TABLE_MAX][CHAR_MAXLEN+1];
int	pos,*kouho_num;
{
	int	i,j;
	...
	i = ejlisp_kouho_next;
	if (pos < 0) {
		if ((i -= ejlisp_kouho_max) < 0)
			i = ejlisp_kouho_num - 
				(ejlisp_kouho_num % ejlisp_kouho_max);
		if ((i -= ejlisp_kouho_max) < 0)
			i = ejlisp_kouho_num - 
				(ejlisp_kouho_num % ejlisp_kouho_max);
	}
...
ejlisp_get_userdic (kouho_arrey,kouho_len_arrey,kouho_entry_arrey)
unsigned short	kouho_arrey[TABLE_MAX][CHAR_MAXLEN+1];
...
ejlisp_get_userdic_name (dic_name_array,dic_name_len_array,dic_no_array)
unsigned short	dic_name_array[TABLE_MAX][CHAR_MAXLEN+1];
...
ejlisp_get_yomi (bunsetsu_no,yomi,yomi_len)
int	bunsetsu_no;
char	*yomi;
int	*yomi_len;
{
...
ejlisp_open (wnn_env,kouho_max,server_name)
char	*wnn_env,*server_name;
int	kouho_max;
{
...
ejlisp_reconv(bunsetsu_no,bunsetsu_len,kanji,kanji_len,bunsetsu_suu)
int	bunsetsu_no,bunsetsu_len;
unsigned short	kanji[];
int	*kanji_len,*bunsetsu_suu;
{
	...
	wnn_errorno = 0;
	TIMEOUT(*bunsetsu_suu = jl_nobi_conv (ejlisp_buf,bunsetsu_no,bunsetsu_len,-1,WNN_USE_MAE,WNN_SHO));
	if (wnn_errorno == 0) {
		TIMEOUT(*kanji_len = jl_get_kanji (ejlisp_buf,0,-1,kanji)) ;
	}
...
ejlisp_search_userdic (dic_no,yomi)
int	dic_no;
unsigned short	*yomi;
{
...
ejlisp_select_bunsetsu(bunsetsu_no,bunsetsu_len)
int	bunsetsu_no,*bunsetsu_len;
{
	...
	if (wnn_errorno == 0) {
		TIMEOUT(jl_zenkouho (ejlisp_buf,bunsetsu_no,WNN_USE_MAE,WNN_NO_UNIQ));
	}
	if (wnn_errorno == 0) {
...
ejlisp_select_kouho (kouho_no,kanji)
int	kouho_no;
unsigned short	*kanji;
{
	int	no,i,c;
	...
	if (wnn_errorno == 0) {
		TIMEOUT(jl_get_zenkouho_kanji(ejlisp_buf,no,kanji));
	}
